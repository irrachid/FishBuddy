...
public class MapOverlayLayout<V extends MarkerView> extends FrameLayout implements MarkerClickListener {
    ...
    protected void addMarker(final V view) {
        view.setOnMarkerPositionClickListener(this);
    ...
    public void showSelectedMarker(final int position) {
        for(int i = 0; i < markersList.size(); i++){
            if(position != i){
                markersList.get(i).setUnselectedColor();
            }else{
                markersList.get(position).setSelectedColor();
            }
        }
    }

    public void hideUnselectedMarker(final int position) {
        for(int i = 0; i < markersList.size(); i++){
            if(position != i){
                markersList.get(i).hide();
            }else{
    ...
    public void onClick(int position) {
        onMarkerClickListener.onMarkerClick(position);
    }
